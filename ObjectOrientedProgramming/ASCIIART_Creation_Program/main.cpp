...
int main(int argc, char *argv[]){
    ...
    int width = tokens[0];
    int height = tokens[1];
    vector<int> vals(tokens.begin() + 2, tokens.end()); // SINCE MY COMPILER IS NOT SUPPORTING C++11, MODIFIED ONLY THIS NOTATION. 
    ...
    } else if (style_target == "my_style") {
        style = new my_style(width, height, vals);
    }
    else {
    throw; }
    // CREATE DRAWER
