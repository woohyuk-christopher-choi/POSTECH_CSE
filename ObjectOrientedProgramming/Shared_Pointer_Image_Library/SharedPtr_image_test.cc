...
void inverted_draw_ascii_art(const Image<float>& img)
{
	static const char letters[] = "@&%QWNMB0gB#$DR8mHKAXUbp4d9h6PkqwSE2]ayjxY5Zoen[ult13If}C{iF|(7J)vTLS?s/z*cr!+><;=^,_':-.`";
	const int n_letters = sizeof(letters) / sizeof(letters[0]);
	float minv, maxv;
	minv = maxv = img[0][0];
	for(int y = 0; y < img.height(); y++) {
